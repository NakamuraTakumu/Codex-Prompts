...
description: agent 向けテキスト指示（skill / slash command / task プロンプト / AGENTS.md 節 / developer instructions / コード生成プロンプト）を、バイアスを排した実行者に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭打ちになるまで回す。プロンプトや skill を新規作成・大幅改訂した直後、またはエージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいときに使う。
---
...
0. **Iteration 0 — description と body の整合チェック**（静的、subagent 不要）
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。Codex では `spawn_agent` で **新規 subagent を起動** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は、独立した subagent を並列に走らせる。subagent 起動不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（subagent runtime が usage メタを返すなら `tool_uses` を使う。Read / Grep も含める。返さない環境では `N/A` とする）
     - 所要時間（subagent runtime が usage メタを返すなら `duration_ms` を使う。返さない環境では `N/A` とする）
...
- `CLAUDE.md` は Codex では通常 `AGENTS.md` に読み替える
- `Task tool で dispatch` は Codex では通常 `spawn_agent` / `wait_agent` / `close_agent` に読み替える
- ただし Codex では、subagent は **ユーザーが明示的に subagents / delegation / parallel agent work を許可した場合のみ** 使う
- その許可が無いときは、この skill の empirical 部分は保留し、`Iteration 0` と構造審査モードだけを使う
...
呼び出し側はレポートから自己申告部分を抽出し、runtime が usage メタを返すなら `tool_uses` / `duration_ms` を取得して評価軸表を埋める。返さない環境では、質的評価を主に使い、該当列は `N/A` とする。

## 環境制約

新規 subagent を起動できない環境（既に subagent として動作している、`spawn_agent` が使えない、またはユーザーが current chat で subagent を明示許可していない等）では、本 skill の empirical 部分は **適用しない**。
- 代替案 1: 親セッションのユーザーに、current chat で subagent 利用を明示許可してもらう
- 代替案 2: 親セッションのユーザーに別 Codex CLI / app セッションを起動して依頼してもらう
- 代替案 3: 評価を諦め、ユーザーに「empirical evaluation skipped: subagent unavailable」と明示報告する
...
| 出てくる合理化 | 実態 |
|---|---|
| 「自分で読み直せば同じ効果がある」 | 直前に書いた文章を "客観視" はできない。必ず新規 subagent を起動する。 |
...
| 「同じ subagent を使い回そう」 | 前回の改善を学習している。毎回新規に起動する。 |
...
- skill 作成や更新の基本設計は `skill-creator` を参照する
- タスク後の学び固定化は、別の retrospective / documentation workflow と役割を分ける
- 複数シナリオを並列で走らせる場合でも、Codex では current chat での明示許可が前提になる
